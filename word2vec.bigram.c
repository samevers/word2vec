...
void LearnVocabFromTrainFile() {
  char word[MAX_STRING];
  char word_pre[MAX_STRING];
  char word_bigram[MAX_STRING];
  FILE *fin;
  long long a, i;
  for (a = 0; a < vocab_hash_size; a++) vocab_hash[a] = -1;	// 初始化 vocab_hash 
  fin = fopen(train_file, "rb");							// 打开 训练语料
  if (fin == NULL) {
    printf("ERROR: training data file not found!\n");
    exit(1);
  }
  vocab_size = 0;
  AddWordToVocab((char *)"</s>");					// 添加标记符号 </s>
  int isFirst = 0;
  while (1) {
   	ReadWord(word, fin);							// 读取一个word
	if(isFirst == 0)
	{
    	PreWord(word, word_pre);						// 再读取一个word, 以跟前一个word 构成 2-gram。
		isFirst = 1;
		continue;
	}
    //printf("word_pre : %s\n", word_pre);
    //printf("word : %s\n", word);
	combineBigram(word_pre, word, word_bigram);		// 组成 2-gram
    if (feof(fin)) break;
    train_words++;
    ...
    i = SearchVocab(word_bigram);
    if (i == -1) {									// 将word 添加到vocab 里
      a = AddWordToVocab(word_bigram);
      vocab[a].cn = 1;
    } else vocab[i].cn++;							// 或者cn++
    if (vocab_size > vocab_hash_size * 0.7) ReduceVocab();

	PreWord(word, word_pre);							// 再读取一个word
  }
  SortVocab();
  if (debug_mode > 0) {
